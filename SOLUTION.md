...
## Summary
The overarching approach is to use the "Medallion Architecture" as explained below:
...
```mermaid
graph LR;
raw[Raw JSON]-->BRONZE[Bronze Storage]-->SILVER[Silver Cleaned Datasets]-->GOLD[Gold Analytics Data];
```

Each tier builds on the previous using broadly the same architecture:  
```mermaid
graph LR;
pt[PREVIOUS TIER - Iceberg over MinIO]-->spark;
spark[Apache Spark]-->nt[NEXT TIER - Iceberg over MinIO];
```

The SILVER tier builds on the BRONZE tier and produces more usable tables. In this tier I have begun this process by flattening out the top level `ARRAY_TYPE` and `STRUCT_TYPE` fields, to make each field more easily addressable. By flattening this table we produce a very wide table. Producing a large wide table for this tier is an intentional decision due to the nature of Apache Iceberg. In a traditional RDBMS, it is best practice to normalise data to reduce data redundancy and allow for easier schema extension. Because Apache Iceberg supports schema evolution, and is a compressed Parquet format this requirement is no longer needed. Keeping data in a single wide table means that joins can be minimised in subsequent layers which are expensive operations.
...
Apache Spark is a widely used data processing toolchain, built on top of the Hadoop ecosystem. This allows MapReduce style operations to be expressed as simple DataFrame transformations, which are easy to use as a developer. Equally Apache Spark provides a SQL API which can be used to execute ANSI SQL against the data, if desired.  
...
Run up the docker-compose stack. This will also upload the data from your local directory to the MinIO instance.  
...
# Future Enhancements
1. Update to Spark Structured Streaming so that the Apache Iceberg will continuously update.
1. Demonstrate Apache Iceberg time-travel, compaction etc...
1. Integration tests.
